...
public class DiscoveryPlanner implements AIPlanner {
	...
	void compute() {
		...
		for (final AIPlanet planet : world.enemyPlanets) {
			...
			if (sat != null) {
				...
				if (sat != null) {
					final ResearchType sat0 = sat;
					Production prod = world.productions.get(sat);
					if (prod != null) {
						setProduction(sat0);
						return;
					}
					...
					for (ResearchType rt : world.productions.keySet()) {
						...
						if (rt.category.main == ResearchMainCategory.SPACESHIPS) {
							prodCnt++;
	 ...
	 * @param sat0 the technology
	 */
	void setProduction(final ResearchType sat0) {
		final int count = Math.min(1, world.global.spaceshipActive / sat0.productionCost / w.params().productionUnit());
		add(new Action0() {
			@Override
			public void invoke() {
				controls.actionStartProduction(sat0, count, 50);
			}
		});
	}
	/**
