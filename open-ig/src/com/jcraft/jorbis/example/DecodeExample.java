 ...
 * @author ymnk
 */
public final class DecodeExample {
	/** Private constructor. */
	private DecodeExample() {
		// utility program
	}
	/** Conversion size. */
	...
	/**
	 * Main program.
	 * @param arg one optional argument containing the input file
	 */
	public static void main(String[] arg) {
		...
		while (true) { // we repeat if the bitstream is chained
			...
			// decoding
			byte[][] cptr = vc.userComments;
			for (int j = 0; j < cptr.length; j++) {
				if (cptr[j] == null) {
					break;
				}
				System.err.println(new String(cptr[j], 0, cptr[j].length - 1));
			}
