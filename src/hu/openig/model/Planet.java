...
public class Planet implements Named, Owned, HasInventory, HasPosition {
     ...
     */
    protected static void addProduction(Player owner, ProductionStatistics out, Building b,
            double eff, Traits trs) {
        double productionBuff = 1;
        if (!owner.isMainPlayer) {
            productionBuff += owner.world.customBalanceSettings.nonPlayerProductionBoostPercent / 100d;
        }
        if (b.hasResource("spaceship")) {
            double value = b.getResource("spaceship") * eff;
            out.spaceship += trs.apply(TraitKind.SHIP_PRODUCTION, 0.01d, value) * productionBuff;
        }
        if (b.hasResource("equipment")) {
            double value = b.getResource("equipment") * eff;
            out.equipment += trs.apply(TraitKind.EQUIPMENT_PRODUCTION, 0.01d, value) * productionBuff;
        }
        if (b.hasResource("weapon")) {
            double value = b.getResource("weapon") * eff;
            out.weapons += trs.apply(TraitKind.WEAPON_PRODUCTION, 0.01d, value) * productionBuff;
        }
    }
    /**
    ...
    public PlanetStatistics getProductionStatistics() {
        ...
        for (Building b : surface.buildings.iterable()) {
            ...
            if (Building.isOperational(eff)) {
                addProduction(owner, result.activeProduction, b, eff, owner.traits);
            }
            addProduction(owner, result.production, b, 1d, owner.traits);
        }
        return result;
    }
    /**
    ...
    public static boolean canBuild(Planet planet,
        ...
        return (hubFound != "MainBuilding".equals(bt.kind)) && count < typeLimit;
    }
